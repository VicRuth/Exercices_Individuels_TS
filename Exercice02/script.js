...
*/
// Etape 5:
var targetNumber = null; // Nombre à deviner
var attempts = 0; // Nombre de tentatives
//Fonction pour demander au joueur 1 de définir un nombre à deviner.
function askNumber() {
    var givenNumber = 0;
    do {
        var input = prompt("Joueur 1, entrez un nombre compris entre 0 et 50 :");
        if (input === null) {
...
function updateUI(message) {
    ...
    if (attemptsEl)
        attemptsEl.textContent = "Tentatives : ".concat(attempts);
}
/**
...
function endGame() {
    ...
    if (gameArea)
        gameArea.innerHTML = ""; // Effacer la zone de jeu
    var messageEl = document.getElementById("message");
    if (messageEl)
        messageEl.textContent = "Bravo ! Vous avez trouvé le nombre.";
}
/**
...
function gamePlay() {
    ...
    var inputEl = document.getElementById("input-number");
    if (validateBtn && inputEl) {
        validateBtn.addEventListener("click", function () {
            var guessedValue = parseInt(inputEl.value, 10);
